...
Get the sum of all columns with the same name.  This is a really horrible way to do this.  If I had more time I would 

#make list of all unique column names
#grep that name, grep("A2", names(countData))
#add those columns together to a new name and data frame

But I don't, at least it is reproducible fast. 
...
Now I have to rename the  columns based on the library samples.
...
I need to loop over names(countData.2), grep sampleNames[,1], returns row?, and replace with corresponding sampleNames[row,2]. Take it one step at a time. This is so frustrating, but it is really something I need to learn. Duh.  

```{r}
for (x in names(countData.2)[2]) {
  sample <- grep(x, sampleNames[,1])
  sampleNames[sample,2])
}

#Now all I need to do is modify so it is in a list. 
```
